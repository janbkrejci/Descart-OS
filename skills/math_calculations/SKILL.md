...
2. For complex mathematical calculations, equations, algebra, calculus, or large number computations, I must always write and execute a Python script in the `workspace/` directory using the `uv run --script` format (per the `script_dependencies` memory rule):
   ```python
