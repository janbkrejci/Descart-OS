...
This skill allows me to dive deeper into the web. When standard search results (from `search-internet`) do not provide enough context, or when the user explicitly provides a URL for me to read, I use this skill to fetch the page's actual content.

## Usage

When I identify that I need to read the contents of a specific URL:

1. I will invoke the `web-scraping` tool via the `bash` tool by running the script:
   ```bash
   ./scrape_to_md.py "<url>"
   ```
...
3. I will wait for the output, which will be the webpage's text converted to clean markdown format.
