...
1. I will write a Python script in the `workspace/` directory using the `uv run --script` format (per the `script-dependencies` memory rule):
   ...
   # ///
   ```
2. For **reading** text from a PDF, I will use `pdfplumber`. For **creating** PDFs, I will use `reportlab`. For **converting** existing files between PDF and other formats, I can use `pandoc` or a combination of tools as needed.
3. I will make the script executable with `chmod +x` and execute it via the `bash` tool.
4. I will relay the extracted information or confirm the generation or conversion of the PDF to the user.
